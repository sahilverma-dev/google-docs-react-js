...
const Home = () => {
    ...
    const createDoc = async () => {
        ...
        const docRef = await addDoc(collection(firestore, "userDocs", `${user?.uid}`, "docs"), {
            name: `${input}`,
    ...
    const model = (
        <Modal size="sm" active={showModel} toggler={() => setShoModel(false)}>
            ...
            <ModalFooter>
                ...
                <Button color="blue" onClick={createDoc} ripple="dark">
                    Create
                </Button>
            </ModalFooter>
        </Modal>
    ...
    useEffect(() => {
        const getData = async () => {
            ...
            if (docSnap) {
                setUserDoc(docSnap.docs.map(doc => {
                    return ({
                        id: doc.id,
                        ...doc.data()
                    });
    ...
    return (
        <>
            <Header />
            {model}
            <section style={{ background: "#f8f9fa" }} className="bg-[#f8f9fa] pb-10 px-10 ">
            ...
            <section className="bg-white px-10 mb-20">
                ...
                {
                    userDoc.length === 0 ? <div className="w-full text-center py-5">No documents</div> : ""
                }
